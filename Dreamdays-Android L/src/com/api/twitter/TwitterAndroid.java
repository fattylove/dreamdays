			...
			new Thread( new Runnable() {
				public void run() {
					...
					try {
						...
						SessionState sessionState = SessionState.create( SessionState.CONNECTED, new Session( token, userId, screen_name ) );
						setSessionState( sessionState );
						...
						if ( e.isCausedByNetworkIssue() ) {
							...
							if ( null != screenName && userId > 0 ) {
								SessionState sessionState = SessionState.create( SessionState.CONNECTED, new Session( token, userId, screenName ) );
								setSessionState( sessionState );
							}
							return;
						}
