 ...
 */
//TODO style nothing
public class ALVoice_EActivity extends Activity implements OnClickListener{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (matterID == 0) {
			...
			matterID = matterDB.getMaxId(ALVoice_EActivity.this) + 1;
		}
			...
			if (mRecordState == RECORD_ING) {
				...
				if (existSDCard()) {
					...
					if (mRecordTime <= MIN_TIME) {
						Toast.makeText(ALVoice_EActivity.this,R.string.app_record_tooshort,Toast.LENGTH_SHORT).show();
						mRecordTime = 0;
					}else{
						intent.putExtra("path", mRecordPath);
						ALVoice_EActivity.this.setResult(RESULT_OK, intent);
						saveVoice(mRecordPath);
						finish();
					}
				} else {
					Toast.makeText(ALVoice_EActivity.this,R.string.app_record_sdcarderror, Toast.LENGTH_SHORT).show();
				}
			}
			break;
		}
	...
	public void saveVoice(String vPath){
		MatterService matterDB = new MatterService();
		matter = matterDB.queryMatterById(ALVoice_EActivity.this, matterID);
		if(matter!=null){
			matter.setVideoName(vPath);
			matterDB.insertOrUpdateNewMatter(ALVoice_EActivity.this, matter);
		}
	}
